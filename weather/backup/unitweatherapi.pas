...
var
  ...
  jItem: TJSONData;
begin
  for I := 0 to AjObject.Count-1 do
  begin
    jItem := AjObject.Items[I];
    LTemp := TForecastData.Create;
    LTemp.MaxTempC := jItem.FindPath('maxtemp_c').AsFloat;
    LTemp.MinTempC := jItem.FindPath('mintemp_c').AsFloat;
    LTemp.AvgTempC := jItem.FindPath('avgtemp_c').AsFloat;
    LTemp.TotalPrecipMM := jItem.FindPath('totalprecip_mm').AsFloat;
    LTemp.WillItRain := jItem.FindPath('daily_will_it_raim').AsInteger = 1;
    LTemp.ChanceofRain := jItem.FindPath('daily_chance_of_raim').AsInteger;
    LTemp.ConditionText := jItem.FindPath('condition.text').AsString;
    LIcon := AjObject.FindPath('condition.icon').AsString;
    if LIcon <> '' then
